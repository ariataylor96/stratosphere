...
def title(term, split):
    return term.cyan(term.bold + term.underline + term.center(split.name)) + term.normal
...
def ui_loop(term, data):
    ...
    while True:
        ...
        if lower == " " and split_index < len(data.splits) - 1:
            split_index += 1

        if key.code == term.KEY_F8 and split_index > 0:
            split_index -= 1
